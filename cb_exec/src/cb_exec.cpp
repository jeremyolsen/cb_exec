...
bool CBExec::Drone::init(ros::NodeHandle& nh)
{
    is_initialized_ = true;
    return true;
}

void CBExec::Drone::executeCommand(const CBExec& ctl, const geometry_msgs::PoseStamped& goto_pose,
...
bool CBExec::APMRoverRC::init(ros::NodeHandle& nh)
{
...
void CBExec::APMRoverRC::printArgs()
{
...
void CBExec::APMRoverRC::executeCommand(const CBExec& ctl, const geometry_msgs::PoseStamped& goto_pose,
                                             float linear_control_val, float angular_control_val, bool has_command)
{
    ROS_ASSERT(is_initialized_);
...
bool CBExec::APMRoverWaypoint::init(ros::NodeHandle& nh)
{
    is_initialized_ = true;
    return true;
}

void CBExec::APMRoverWaypoint::executeCommand(const CBExec& ctl, const geometry_msgs::PoseStamped& goto_pose,
                                          float /*linear_control_val*/, float /*angular_control_val*/, bool /*has_command*/)
{
    ROS_ASSERT(is_initialized_);
    // Publish pose update to MAVROS
...
void CBExec::navioStateCallback(const mavros_msgs::State::ConstPtr &msg)
{
    fcu_state_ = *msg;
}

void CBExec::poseCallback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
...
void CBExec::joystickCallback(const sensor_msgs::Joy::ConstPtr& msg)
{
...
void CBExec::dnnCallback(const sensor_msgs::Image::ConstPtr& msg)
{
...
void CBExec::objDnnCallback(const sensor_msgs::Image::ConstPtr& msg)
{
...
void CBExec::computeDNNControl(const float class_probabilities[6], float& linear_control_val, float& angular_control_val)
{
...
bool CBExec::parseArguments(const ros::NodeHandle& nh)
{
...
bool CBExec::init(ros::NodeHandle& nh)
{
    ...
    fcu_state_sub_ = nh.subscribe<mavros_msgs::State>("/mavros/state", QUEUE_SIZE, &CBExec::navioStateCallback, this);
    if(!fcu_state_sub_)
    {
    ...
    local_pose_sub_ = nh.subscribe<geometry_msgs::PoseStamped>("/mavros/local_position/pose", QUEUE_SIZE, &CBExec::poseCallback, this);
    if(!local_pose_sub_)
    {
    ...
    joy_sub_ = nh.subscribe<sensor_msgs::Joy>("/joy", command_queue_size_, &CBExec::joystickCallback, this);
    if(joy_sub_)
    {
    ...
    dnn_sub_ = nh.subscribe<sensor_msgs::Image>("/trails_dnn/network/output", command_queue_size_, &CBExec::dnnCallback, this);
    if(dnn_sub_)
    {
    ...
    objdnn_sub_ = nh.subscribe<sensor_msgs::Image>("/object_dnn/network/output", command_queue_size_, &CBExec::objDnnCallback, this);
    if(objdnn_sub_)
    {
...
bool CBExec::setJoystickParams(std::string joy_type)
{
...
void CBExec::spin()
{
    ...
    {
        ROS_INFO("Cannot spin in CBExec node! Vehicle is not armed!");
        return;
    }
    ...
    ROS_INFO("CBExec is processing requests...");
    ...
    while (ros::ok())
    {
        ...
        default:
            ROS_ERROR("CBExec::spin detected unknown state: %d!", (int)controller_state_);
            break;
        }
...
inline geometry_msgs::Quaternion CBExec::getRotationTo(geometry_msgs::Point& position, geometry_msgs::Point& target_position) const
{
...
inline geometry_msgs::Point CBExec::computeNextWaypoint(
...
inline float CBExec::getPoseDistance(geometry_msgs::PoseStamped& pose1, geometry_msgs::PoseStamped& pose2) const
{
...
inline float CBExec::doExpSmoothing(float cur, float prev, float factor) const
{
